...
- **Motivo:** Simplifica o roteamento em GitHub Pages (sem configuração de redirecionamento) e mantém o contexto do Firebase Auth entre telas.
...
users/{uid}/
  obras/{obraId}/
    cronograma[]               → planejamento mensal consolidado
    cronogramaExecucao[]       → execução mensal consolidada
    itens[]                    → serviços com peso e valor
    itensCronograma[]          → cronograma mensal por serviço
    itensCronogramaExecucao[]  → execução mensal por serviço
    resumo{}                   → totais consolidados
    dataInicio                 → string 'YYYY-MM'
    dataEmissaoExecucao{}      → { mes, ano } da última atualização
    historicoExecucao[]        → versões anteriores de execução
    aditivos[]                 → aditivos com cronograma próprio
```
...
- **Motivo:** Acesso direto por índice sem necessidade de busca. Consistente com o cálculo de labels em `buildCronogramaTimeline` e `buildCurvaServico`.
- **Atenção:** O gráfico usa índice `offset = 0..n-1` com `base0 = (iniMes-1) + offset` para calcular o label correto do mês.
...
- **Motivo:** Obras públicas já entregam o cronograma em planilha. Evita digitação manual de dezenas de linhas.
...
- **Motivo:** Leve, sem dependências, funciona com canvas nativo, suporte a linha com `spanGaps: false` para interromper a linha em pontos `null`.
- **Alternativas descartadas:** D3.js (muito complexo para o caso de uso), ApexCharts (mais pesado).

### Ponto de origem das Curvas S é "Mês 0" com valor zero
- **Decisão:** O primeiro ponto do gráfico é sempre um ponto "Mês 0" com 0%, antes do mês 1 do cronograma.
- **Motivo:** A curva S precisa partir do zero visualmente. Sem esse ponto, a linha começa flutuando no valor do primeiro mês.

### Planejado nunca é cortado
- **Decisão:** A linha laranja (planejado) sempre exibe todos os meses do cronograma, do primeiro ao último.
- **Motivo:** O usuário precisa ver o horizonte completo do contrato para comparar com o realizado.
...
- **Comportamento anterior (descartado):** O corte era feito pelo último mês com `planejadoPct > 0`. Isso causava um platô horizontal na linha verde até o fim do contrato mesmo sem execução.
- **Algoritmo definido:**
  ...
  4. Buracos no meio (mês zero entre execuções) são **preservados** — a linha continua normalmente após o buraco.
- **Onde está implementado:**
  - Curva S por Serviço → `state.js` → função `buildCurvaServico`
  - Curva S do Contrato e Aditivo → `render-charts.js` → função `_renderCurvaS2Generica`
- **Regra permanente:** Esta lógica deve ser idêntica nos dois lugares. Se alterar um, alterar o outro.
...
- **Decisão:** Antes de sobrescrever a execução mensal, a versão atual é salva em `historicoExecucao[]` no Firestore.
- **Motivo:** Permite comparar a curva do executado atual com versões anteriores (linha pontilhada cinza no gráfico).
- **Campo de referência:** `dataEmissaoExecucao { mes, ano }` identifica a qual competência pertence cada versão.
...
- **Decisão:** O sistema detecta e exibe alertas para três tipos de anomalia por serviço:
