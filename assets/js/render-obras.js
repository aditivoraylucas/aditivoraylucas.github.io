...
export function renderCronogramaMensalBox(){
  ...
  if(tem){
    const emissaoTxt=o.dataEmissaoExecucao?` \u00b7 Emiss\u00e3o: <strong>${String(o.dataEmissaoExecucao.mes).padStart(2,'0')}/${o.dataEmissaoExecucao.ano}</strong>`:'';
    box.innerHTML=
...
export function renderObrasBox(){
  ...
  $('deleteObraBtn').onclick=async()=>{
    ...
    const nome=currentObra()?.nome||'esta obra';
    ...
    if(state.selectedObraId){ const prox=state.obras.find(x=>x.id===state.selectedObraId); if(prox) applySelected(prox); }
    else{ state.rows=[]; ['projName','projContratada','projScope'].forEach(k=>{ const el=$(k); if(el) el.value=''; }); limparObraIdDaUrl(); }
    renderAll(); showToast(`"${nome}" removida.`);
  };
  renderCronogramaBox(); renderCronogramaMensalBox(); renderAditivosSection();
}
