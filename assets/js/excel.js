...
/**
 * Localiza a linha sentinela onde "ITEM" aparece como cabeçalho da tabela.
 * Todo conteúdo abaixo desta linha é dado de serviço — não cabeçalho do BM.
 */
...
/**
 * Extrai o valor monetário de "Esta Medição" do cabeçalho do BM.
 *
 * Regras:
 * 1. Busca apenas acima da linha sentinela "ITEM" (exclusive) — evita
 *    capturar "MEDIÇÃO Nº" que pertence ao cabeçalho da tabela de serviços.
 * 2. Aceita somente rótulos que identificam o valor desta medição:
 *    "esta medição" ou "medição atual" — nunca "medição nº" ou "medição n°".
 * 3. O valor está na mesma coluna, linha seguinte (padrão BM), ou à direita
 *    na mesma linha. Deve ser > 1 para excluir o número ordinal da medição.
 */
function findEstaMedicao(rows){
  ...
  for(let r = 0; r < limiteSeguro; r++){
    ...
    for(let c = 0; c < row.length; c++){
      ...
      // Valor na mesma coluna, linha seguinte (padrão BM)
